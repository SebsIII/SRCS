    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.println("<p style='color:white;font-size:20vw;margin:0px;' > SRCS </p>");
          client.println("<h3 style='color:white;margin:0px;' > Main Dashboard </h3>");
          client.println("<button onclick=X()>Test</button>");

          client.println("<script>function X(){document.querySelector('body').style.backgroundColor = 'blue'}</script>");
